...
배치 애플리케이션은 내트워크 장애 등으로 실패했을 때 **언제든 안전하게 재실행(Re-run)** 가능해야 하며, **여러 번 실행하더라도 결과는 항상 같아야 합니다.**
...
- 매일 새벽 배치에서 사용자의 **전체 활동 데이터**를 다시 수집 → **점수 덮어쓰기(Overwrite)**
...
약 27시간 47분이 소요됩니다. 매일 자정에 도는 배치가 다음날 자정까지도 끝나지 않는 상황이기 때문에 서비스 운영이 불가능한 구조입니다.
...
- REST API 5회 호출을 단 한번의 GraphQL 쿼리로 통합하여 네트워크 오버헤드 감소
